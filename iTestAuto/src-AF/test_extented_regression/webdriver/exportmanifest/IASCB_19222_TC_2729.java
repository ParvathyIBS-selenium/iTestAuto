...
/**
 * 
 * 2729 -Verify that flight type is displayed in red when flight type is changed
 *
 *
 * 
 */

public class IASCB_19222_TC_2729 extends BaseSetup {
	...
	public void getTestSuite(Map<Object, Object> map) throws IOException {
		...
		try {
			...
			// Login to "MERCURY"
			mercury = libr.getApplicationParams("mercury");
			driver.get(mercury[0]); // Enters URL
			cust.loginToMercury(mercury[1], mercury[2]);

			cust.createTextMessage("MessageExcelAndSheetSSM2", "MessageParamSSM2");
			mercuryScreen.clickSendMessage();
			mercuryScreen.enterTelexAddress("SenderAddressMercury", "DestinationAddressMercury", true);
			mercuryScreen.sendMessageInMercury();
			mercuryScreen.verifyMsgStatus("SSM");
