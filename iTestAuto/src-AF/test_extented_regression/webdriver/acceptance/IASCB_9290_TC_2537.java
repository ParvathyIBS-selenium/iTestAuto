...
/** TC_01_Retrieving actual weight of ULD from weight scale. **/
...
public class IASCB_9290_TC_2537 extends BaseSetup {
	...
	public void getTestSuite(Map<Object, Object> map) throws Exception {
		...
		try {
			...
			MSG005.verifyMessageDetails(verfCols, actVerfValues, pmKeyXFWB,"val~XFWB",false);
			...
			MSG005.verifyMessageDetails(verfCols, actVerfValues, pmKeyRCS,"val~XFSU-RCS",false);
			MSG005.closeTab("MSG005", "MSG005 - List Messages");
			...
			//Capturing contour and saving shipment details
			...
			buhht.enterWeightCaptureLocation("BuildUpLoc");
			String tarewgt = buhht.getTareWeight("tareweight");	
			map.put("ActualScaleWeight",String.valueOf(Integer.parseInt(cust.data("Weight"))+Integer.parseInt(tarewgt)+Integer.parseInt(cust.data("ExtraWeight"))));

			/******* POST REQUEST - Weight is received for Pit Location ****/	
			libr.waitForSync(3);
			jsonbody.postRequest_Export(cust.data("EquipmentID"),cust.data("UldNum"),cust.data("WeighScaleId"),cust.data("ActualScaleWeight"),cust.data("Height"),cust.data("ForwardLinkLen"),cust.data("AfterLinkLen"),cust.data("LeftLinkLen"),cust.data("RightLinkLen"));
			libr.waitForSync(8);

			//Retrieving Actual Weight from service
			buhht.clickGetScaleWeight();
			buhht.verifyScaleWeightAutopopulated("ActualScaleWeight");
			buhht.verifyWeightScaleIDAutopopulated("WeighScaleId");
			buhht.enterUldHeight("Height");
			buhht.clickWeightCaptureSave();
			libr.quitApp();	

			/***  SHR011 - List Audit Enquiry screen ***/
			//Verify the actual weight received is displayed in SHR011 screen
			cust.searchScreen("SHR011", "List Audit Enquiry");
			SHR011.selectModuleName("Operations");
			SHR011.selectSubModuleName("ULD");
			String currDate=cust.createDateFormatWithTimeZone("dd-MMM-YYYY", 0, "DAY", "Europe/Paris");
			SHR011.enterFromDate(currDate);
			SHR011.enterToDate(currDate);
			SHR011.enterOpsULDDetails("UldNum","carrierCode", "FlightNo", "StartDate");			
			SHR011.listDetails();
			map.put("pmKey","Scale weight : "+cust.data("ActualScaleWeight")+".0");
			int verfCol[]={1,3,4,5,5,6};
			String actVerfValue[]={"ACTWGTUPD",hht[0],currDate,"Scale weight : "+cust.data("ActualScaleWeight")+".0","Actual Weight="+cust.data("ActualScaleWeight")+".0",cust.data("Origin")};
			SHR011.verifyTransactionDetailsValue(verfCol, actVerfValue, cust.data("pmKey"));
