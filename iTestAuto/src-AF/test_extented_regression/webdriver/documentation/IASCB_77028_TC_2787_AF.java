...
/**
 * 
 *  TC_08_Charge code logic when AWB is created from OPR026 with CC payment type
 *
 *
 */

public class IASCB_77028_TC_2787_AF extends BaseSetup {
	...
	public void getTestSuite(Map<Object, Object> map) {
		...
		try {
			...
			OPR026.changePaymentType("PaymentType");
			OPR026.saveAWB(); 
			...
			OPR026.verifyChargeCode("ChargeCode");
			//add semi automatic charge details	
		    OPR026.enterChargeCode(0,"val~CA");
			...
			//verify semi automatic and automatic charge detail verification as per configuration
			cust.compareMaps(hm, expValues,"OPR026","Other Charges");
			OPR026.clickComputeTotal();
			OPR026.asIsExecute();
			cust.closeTab("OPR026", "Capture AWB");

			/**	Verifying charge code in OPR026***/
			cust.searchScreen("OPR026", "Capture AWB");
			OPR026.listAWB("AWBNo", "CarrierNumericCode");
			OPR026.clickChargesAcc();
			OPR026.verifyChargeCode("ChargeCode");
			...
			//successfully verified the automatic charge RA populated for DG shipment  after Execution done
			CSH007.verifychargespopulated(cust.data("OtherCharges").split(",")[0],cust.data("OCValue").split(",")[0]);
			//successfully verified semi automatic charges populated after Execution done
			CSH007.verifychargespopulated(cust.data("OtherCharges").split(",")[1],cust.data("OCValue").split(",")[1]);
