...
waitTillScreenload(sheetName, "txt_remarksScribblePad;xpath", "Remarks Section code", screenName);
...
* @throws Exception
	 ...
	 * @param milestoneCalc
	 * @return
	 * Desc : calculate CIQ time
	 */
	public Date CIQCalculation(String ata,int milestoneCalc)
	{
		try
		{
			//CIQ time calculation 
			String CIQtimeDisplayed=timeCalculation(ata, "dd-MMM-yyyy HH:mm","MINUTE",milestoneCalc);
			Date ciq=new SimpleDateFormat("dd-MMM-yyyy HH:mm").parse(CIQtimeDisplayed);  
			return ciq;
		}
	 ...
	 */
	public void verifyLPSTimeForTransitShipment(Date explpsValue,String uldNo) throws java.text.ParseException, ParseException
	{

		try{

			DateFormat dateFormat = new SimpleDateFormat("dd-MMM-yyyy HH:mm"); 
			String expLPSValue = dateFormat.format(explpsValue);  
			System.out.println(expLPSValue);

			String expDate=expLPSValue.split(" ")[0];
			String expTime=expLPSValue.split(" ")[1];
			...
			//get LPS time displayed on screen
			String locator= xls_Read.getCellValue(sheetName, "txt_lpsTime;xpath").replace("*", data(uldNo));
			String actLPSTime= driver.findElement(By.xpath(locator)).getAttribute("value");

			//get LPS date displayed on screen
			String locator1= xls_Read.getCellValue(sheetName, "txt_lpsDate;xpath");
			String actExplpsDate= driver.findElement(By.xpath(locator1)).getAttribute("value");

			waitForSync(2);
			verifyScreenTextWithExactMatch(sheetName, expTime,actLPSTime , "LPS time displayed","LPS time displayed");
			verifyScreenTextWithExactMatch(sheetName, expDate,actExplpsDate , "LPS time displayed","LPS date displayed");


		}catch (Exception e) {
			writeExtent("Fail", "Failed to verify LPS Time displayed  on "+screenName);
		}
	}
	/**
	 * @author A-9844
	 ...
	 * @param milestoneCalc
	 * @return
	 * Desc : calculate buildup closure
	 */
	public Date buildupClosureTimeCalculation(String std,int milestoneCalc)
	{
		try
		{
			// time calculation 
			String buildupclosuretimeDisplayed=timeCalculation(std, "dd-MMM-yyyy HH:mm","MINUTE",milestoneCalc);
			Date buildUpClosure=new SimpleDateFormat("dd-MMM-yyyy HH:mm").parse(buildupclosuretimeDisplayed); 
			return buildUpClosure;
		}
	...
	public void verifyLPSForTerminalShipment(Date lps)
	{
		DateFormat dateFormat = new SimpleDateFormat("dd-MMM-yyyy HH:mm");  
        String lpsTime = dateFormat.format(lps); 
 ...
 * @throws ParseException
 */
	public void verifyLPSForTerminalShipment(String ata , int milestoneCalc) throws java.text.ParseException, ParseException
	{

		timeCalculation(ata, "dd-MMM-yyyy HH:mm","MINUTE",milestoneCalc);
	}
	/**
	 * @author A-7271
	 * @param timeSlot
	...
	public void enterThruBreakdownLocation() throws Exception {
		...
		String thruLocation= WebFunctions.getPropertyValue(toproppath, "ThruBreakdownLocation_CDG");
 ...
 */
	 ...
	 * @throws ParseException
	 */
	public void verifyLPSForTerminalShipment(String ata , int milestoneCalc,String uldNo) throws java.text.ParseException, ParseException
	{

		try{

			//LPS time calculation 
			String lpstimeDisplayed=timeCalculation(ata, "dd-MMM-yyyy HH:mm","MINUTE",milestoneCalc);
			String expDate=lpstimeDisplayed.split(" ")[0];
			String expTime=lpstimeDisplayed.split(" ")[1];
	...
	public void addBreakdownInstructions(String breakdownInstruction) throws InterruptedException, IOException {
		...
		locator1 = locator1.replace("*", breakdownInstruction);
	...
	public void selectBDInstructionforULD(String uld,String instruction)
	{
		...
		try{         
			...
			if(size==0){
				...
				String locatorBDKInstructionText = xls_Read.getCellValue(sheetName, "lbl_BreakdownInstructionsTag;xpath");
				locatorBDKInstructionText = locatorBDKInstructionText.replace("ULD", data(uld));
