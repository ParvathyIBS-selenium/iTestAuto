...
public class WarehouseShipmentEnquiry_WHS011 extends CustomFunctions {
	...
	/**@author A-10328
	 * Description - SCC to be sorted as per priority
	 * @param SCC
	 * @param SCC1
	 */
	public void verifySCCSorting(String SCC, String SCC1)

	{
		String locator=xls_Read.getCellValue(sheetName,"txt_SCC;xpath");
		String actSCC=driver.findElement(By.xpath(locator)).getText();
		String[] values = actSCC.split(",");
		if(SCC.equals(values[0].trim()) && SCC1.equals(values[1].trim()))
		{
			writeExtent("Pass","The SCCs are sorted based on priority" + SCC + SCC1 +" on " +screenName);
		...
		else
		{
			...
			writeExtent("Fail","The SCCs are not sorted based on priority" + SCC + SCC1+" on " +screenName);
		}
	}
	...
	/**
	 * Desc : Verifying no records found
	 * @author A-9844
	 * @param status
	 * @throws InterruptedException
	 * @throws AWTException
	 */
	public void verifyNoRecoredsFound(String status) throws InterruptedException, AWTException {
		 ...
		 if((driver.findElements(By.xpath(locator)).size()==1)){
			 writeExtent("Pass","Verified inventory details are cleared off after delivery on "+screenName);
		 }
		 
		 else{
			 writeExtent("Fail","Failed to  details are cleared off on "+screenName); 
		 }
	...
	public void saveColmnConfig() throws InterruptedException, IOException {
		clickWebElement(sheetName, "btn_saveCol;xpath", "Save Button",screenId);
	}
	...
	public void listFlight2(String option,String carrierCode,String flightNumber, String flightDate) throws InterruptedException, AWTException, IOException {
		...
		String sheetname="Generic_Elements";
		selectValueInDropdown(sheetName,"lst_flightDirection;name",data(option),"Flight Direction","VisibleText");
		enterValueInTextbox(sheetname, "inbx_carrierCode;xpath",
				data(carrierCode), "Carrier Code", screenId);
		enterValueInTextbox(sheetname, "inbx_flightNumber;xpath", data(flightNumber),
				"Flight Number", screenId);
