 ...
 */
	...
	/**@author A-10328
	 * Description - Verifying storage position 
	 * @param expectedStorageposition
	 */

	public void verifyStoragePosition(String expectedStorageposition)

	{
		String locator=xls_Read.getCellValue(sheetName, "lst_storagepos;xpath");
		String actStoragepos=driver.findElement(By.xpath(locator)).getText();	

		if(actStoragepos.startsWith(expectedStorageposition))
		{
			writeExtent("Pass", "Verified the Storage Position as "+ expectedStorageposition+" for the location on " + screenName);
		}

		else
		{
			writeExtent("Fail", "Storage position displayed for the location is incorrect on" + screenName);
		}
