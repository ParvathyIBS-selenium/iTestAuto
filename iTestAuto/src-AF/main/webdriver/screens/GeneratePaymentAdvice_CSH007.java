 ...
 */
    ...
    /**
     *Description... Verify charges popupated after payment finalize
* @author A-10330
* @param chargecode,charge value
*/
	public void verifychargespopulated(String chargecode,String chargevalue)
	{
             String locator=xls_Read.getCellValue(sheetName, "lst_chargecodes;xpath");
             List<WebElement> cols = driver.findElements(By.xpath( locator));
		...
		try
		{
			...
			for(int i=1;i<=cols.size();i++)
			{
				...
				String locator2=xls_Read.getCellValue(sheetName, "lst_chargecodes;xpath");
				
				locator2="("+locator2+")["+i+"]"+"//td[1]";
		    	
		    	WebElement el=driver.findElement(By.xpath(locator2));

				String chargehead=el.getText();

				System.out.println(chargehead);
				
				if(chargehead.contains(chargecode))
				{
					writeExtent("Pass","successfully verified the charge code" +chargecode+" is popuplated on"+ScreenName);
				}
				WebElement charges=driver.findElement(By.xpath("//tr[contains(@class,'ic-table-row')]["+i+"]//td[1]//following::span[@id='chargeHeadInService_LBL'][1]"));
				System.out.println(charges.getText());
				if(charges.getText().contains(chargevalue))
				
			   writeExtent("Pass","successfully verified the charge value" +chargevalue+" for charge code"+chargecode+" is popuplated on"+ScreenName);
				

			}
			}catch(Exception e)
			{
				writeExtent("Fail","could not verify the charge code" +chargecode+" is popuplated on"+ScreenName);
			}
