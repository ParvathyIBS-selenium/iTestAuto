...
public class TransportOrderListing extends CustomFunctions {
	...
	public String HApath = "\\src\\resources\\HA.properties";
	 ...
	 */
	public void verifySourcelocandDestLocIsDifferent(String srclocation, String destLocation ) {
		try{

			if(!(data(srclocation).equalsIgnoreCase(data(destLocation))))
			{
				writeExtent("Pass","Generated TO origin "+data(srclocation)+" and destination "+data(destLocation)+"are not same");
			}
			else
			{
				writeExtent("Fail","Generated TO origin "+data(srclocation)+" and destination "+data(destLocation)+"are same");
			}
			...
			writeExtent("Fail","Failed to verify the genarated TO origin and destination locations");
		}
	...
	/*** @author A-10328
	 * Desc- To verify No TO dispalyed for departed ULD
	 * @param uldNo/AWB
	 */
	public void verifyNoTOForDepartedULD(String shipment) {
		...
		if(eleSize==0)
		{
			writeExtent("Pass","Verified that the TO "+data(shipment)+" is not triggered for departed ULD ");

		}
		else
		{
			writeExtent("Fail", "TO is triggered for departed ULD " + data(shipment) + " on " + screenName );
	...
	public void enterShipment(String shipment) throws IOException
	{

		String HA_CDG=getPropertyValue(HApath,"HA_Buildup_CDG");
		String HA_AMS=getPropertyValue(HApath,"HA_Buildup_AMS");
		int size = getSizeOfMobileElement("to_selectHAText;xpath", proppathtransportorder);
		waitForSync(3);
		if (size == 1)
		{
	...
	* Desc - Click relocation complete OK button
	...
	public void verifyShipmentIsListed(String shipment) {
		...
		if(eleSize==1)
		{
			writeExtent("Pass","Verified that the SU  "+data(shipment)+" is listed");
