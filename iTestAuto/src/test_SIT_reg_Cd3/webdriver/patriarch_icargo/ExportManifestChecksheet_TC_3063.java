...
/**  TC_03_Check sheet capture at export manifest and verify in Patriarch system.-Split shipment.   **/
...
public class ExportManifestChecksheet_TC_3063 extends BaseSetup {
	...
	public void getTestSuite(Map<Object, Object> map) {
		...
		try {
			...
			//Create XFWB message
			String sccs[] = { libr.data("SCC").split(",")[0], libr.data("SCC").split(",")[1]};
			...
			String wgt[] = { libr.data("Weight1").split(",")[0], libr.data("Weight1").split(",")[1] };
			...
			String locs[]={cust.data("Location"),cust.data("Location1")};
			...
			excelRead.writeDataInExcel(map, path1, sheetName, testName);
			map.put("pcs", cust.data("SplitPcs").split(",")[0]);
			map.put("wt", cust.data("Weight1").split(",")[0]);
			OPR344.assignSplitShipmenttoULD("UldNum","0","CarrierNumericCode","AWBNo","pcs","wt");
			cust.closeTab("OPR344", "Export manifest");
			
			/**** OPR344 - Export manifest****/
			cust.searchScreen("OPR344", "Export manifest");
			OPR344.listFlight("carrierCode", "FlightNo","StartDate");	
			map.put("pcs", cust.data("SplitPcs").split(",")[1]);
			map.put("wt", cust.data("Weight1").split(",")[1]);
			OPR344.addNewULDWithAWBSplitShipment("val~BULK","0","CarrierNumericCode","AWBNo","pcs","wt");
			OPR344.expandULDs();			
			...
			map.put("pmKey",cust.data("val~BULK")+cust.data("CarrierNumericCode")+cust.data("AWBNo")+"11");
			OPR344.clickCheckSheet("pmKey");
			OPR344.captureCheckSheet(true);
