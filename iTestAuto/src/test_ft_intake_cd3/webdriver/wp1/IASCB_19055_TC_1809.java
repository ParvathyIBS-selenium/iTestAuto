...
/**
 *  1809 - TC_02_Verify that fields vehicle number and expiry date are present Without QPU QDO codes
 *  
 * 
 **/


public class IASCB_19055_TC_1809 extends BaseSetup {
	...
	public void getTestSuite(Map<Object, Object> map) {
		...
		try {
			...
			sstDP.verifyIDExpiryDateField();
			sstDP.enterDriverDetailsWithScroll("EndDate");
			...
			//Verify public side token is listed
			/************TGC010- VISIT DECLARATION ENQUIRY*****/
			cust.searchScreen("TGC010","Visit Declaration Enquiry");
			TGC010.enterToken("TokenId");
			TGC010.clickList();
			//Verify Trailer no 1 and Trailer no 2
			int verfCols[]={28}; 
			String trailerNo=cust.data("TrailerNo1")+","+cust.data("TrailerNo2");
			String[] actVerfValues={trailerNo};
			TGC010.verifyVisitDeclarationDetails(verfCols, actVerfValues, cust.data("TokenId"));
			
			//Verify ID expiry date
			String expDate = cust.createDateFormatWithTimeZone("dd-MMM-YYYY", 7, "DAY", "");
			map.put("ExpiryDate", expDate);
			TGC010.verifyExpiryDate("ExpiryDate");
