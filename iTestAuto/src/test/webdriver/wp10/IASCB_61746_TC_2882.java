...
/** TC_05_Verify TO generation to build-up location when screening details are received from RAPIX.  **/
...
public class IASCB_61746_TC_2882 extends BaseSetup {
...
//			gahht.selectHandlingAreaAndClickDone();
			...
			//Verifying destination Control location and zone
			...
			map.put("RapixEntryPoint", WebFunctions.getPropertyValue(toproppath, "RapixEntryLocation"));
			excelRead.writeDataInExcel(map, path1, sheetName, testName);
			WHS009.enterDestinationLocAndSU("RapixEntryPoint","SU");
			...
			OPR339.verifyScreeningResultAndSUnumber(cust.data("ScreeningResult").split(",")[1],cust.data("SU"));			
			...
			/** WAREHOUSE RELOCATION - WHS009 **/
			//Manual relocation to Rapix Exit Point on Screening details receival
			cust.searchScreen("WHS009", "Warehouse Relocation");
			WHS009.enterSU("SU");
			WHS009.listAwbDetails();
			//Full Relocation
			...
			//Verifying Rapix Exit Location and Zone
		
			//Verifying destination Storage location and Zone
			String[] actVerfValue2= {WebFunctions.getPropertyValue(toproppath, "StorageAreaZone_CDG")};
			//verifying the location displayed is in the correct Zone as per the configuration
			WHS008.verifyLocationAndCorrespondingZone("destnStorageLocation", verfCols, actVerfValue2);
			cust.closeTab("WHS008", "Handling Area Set Up");
