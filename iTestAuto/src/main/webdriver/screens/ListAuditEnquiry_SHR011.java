	 ...
	 */
		...
		try{
			  ...
			  writeExtent("Fail", "Failed to select the "+type+" radio button on "+ScreenName);
	            ...
	            /**
	             * Description... wait till the list displays
	             * @throws InterruptedException
	             * @throws IOException 
	             */
	            public void waitTillDetailsDisplayed() throws InterruptedException, IOException
	            {
	            	Boolean detailsDisplayed=true;
	            	int count=1;
	            	while((detailsDisplayed)&&(count<60))
	            		if((driver.findElements(By.xpath("//a[@id='exportToExcelLink']")).size())==1)
	            		{
