...
public class BreakdownHHT extends CustomFunctions {
	 ...
	 * @throws IOException
	 */
	public void clickSaveWithoutLocation() throws AWTException, InterruptedException, IOException
	{
...
public void selectSCC(String sccs[]) throws IOException{
	...
	/*** Commented as SCCs are getting auto selected
	try{
...
clickActionInHHT("breakdownhht_selectScc;xpath",proppathhht,"SCC icon",screenName);
 ...
 waitForSync(7);
	...
	public void clickInstructionIcon(String FullAwb) {
		...
		String Instructionpage = getPropertyValue(proppathhht,"breakdownhht_Instructionpage;xpath");
		int size=androiddriver.findElements(By.xpath(Instructionpage)).size();
		try {
			...
			while(size!=1)
			{
				...
				size=androiddriver.findElements(By.xpath(Instructionpage)).size();
				if(size==1)
					break;
...
public void clickClose() throws IOException {
	clickActionInHHT("breakdownhht_Instructionpageclose;xpath",proppathhht,"Close confirmation message",screenName);	
}
 ...
 * @throws IOException 
	...
	public void clickUnitizedYes() throws AWTException, InterruptedException, IOException
	{
		...
		String inbx_pieces= getPropertyValue(proppathhht,"breakdownhht_inbx_pcs;accessibilityId");

		while( androiddriver.findElementsByAccessibilityId(inbx_pieces).size()==1)
		{
...
/**
	...
	public void enterLocationIFNotDisplayed() throws AWTException, InterruptedException, IOException
	{
		...
		if(androiddriver.findElements(By.xpath(locationDisplayed)).size()==1)	
			getAndEnterLocation();
