package cgoimcom;
/**   TC_03_Verify the location update when ULD exits the IM **/
...
public class RelocateStorageUnit_TC_10773 extends BaseSetup {
	...
	public void getTestSuite(Map<Object, Object> map) {
		...
		try {
			...
			/******* POST REQUEST****/	
			//Trigger RelocateStorageUnit to the entry point of IM with Occupancy status O
			map.put("IMEntryLoc",cust.data("ForwardLocation").split(",")[0]);
			map.put("OccupancyStatusEntry",cust.data("OccupancyStatus").split(",")[0]);
			jsonbody.postRequest(cust.data("EquipmentID"),cust.data("UldNum"),cust.data("IMEntryLoc"),cust.data("OccupancyStatusEntry"),cust.data("WareHouse"));
			excelRead.writeDataInExcel(map, path1, sheetName, testName);
			...
			int verfCol[]={4};  
			String[] actVerfVal={"TE5-"+cust.data("IMEntryLoc")};
			WHS011.verifyWarehouseDetailsWithPmKey(verfCol, actVerfVal,"UldNum");
			libr.quitBrowser();
			
			/******* POST REQUEST****/	
			//Trigger RelocateStorageUnit to the exit point of IM with Occupancu status E
			map.put("IMExitLoc",cust.data("ForwardLocation").split(",")[1]);
			map.put("OccupancyStatusExit",cust.data("OccupancyStatus").split(",")[1]);
			jsonbody.postRequest(cust.data("EquipmentID"),cust.data("UldNum"),cust.data("IMExitLoc"),cust.data("OccupancyStatusExit"),cust.data("WareHouse"));
			excelRead.writeDataInExcel(map, path1, sheetName, testName);
			...
			WHS011.clickAlsoShowEmptySUCheckbox();
			WHS011.clickList();
			//Verify Location of uld at the exit of IM
			String[] actVerfVal1={"TE5-"+cust.data("IMExitLoc")};
			WHS011.verifyWarehouseDetailsWithPmKey(verfCol, actVerfVal1,"UldNum");
			libr.quitBrowser();
