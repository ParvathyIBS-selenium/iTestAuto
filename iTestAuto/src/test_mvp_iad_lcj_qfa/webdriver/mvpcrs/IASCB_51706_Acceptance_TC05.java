...
/** Test Case Name :Verify user can able to Add or Modify the Split Shipment details in Goods Acceptance screen with the new business privilege **/

public class IASCB_51706_Acceptance_TC05 extends BaseSetup {
	...
	public void getTestSuite(Map<Object, Object> map) {
		...
		try {
			...
			MSG005.clickList();
			libr.waitForSync(6);
			map.put("pmkey", "NEW"+" - "+cust.data("prop~flight_code")+" - "+cust.data("FlightNo")+" - "+cust.data("FBLDate").toUpperCase());
	        ...
	        /***** OPR026 - Execute AWB****/
            //Provide booking details and Execute the AWB
			cust.searchScreen("OPR026","Capture AWB");
			OPR026.listAWB("prop~AWBNo", "prop~CarrierNumericCode");	
			OPR026.asIsExecute();
			cust.closeTab("OPR026", "Capture AWB");
			...
			OPR335.verifyLooseShipmentDetails(pcs);
			String pcsinfo[]={libr.data("SplitPcs2").split(",")[0],libr.data("SplitPcs2").split(",")[1],libr.data("SplitPcs2").split(",")[2],libr.data("SplitPcs2").split(",")[3]};
			String locs[]={libr.data("Location2").split(",")[0],libr.data("Location2").split(",")[1],libr.data("Location2").split(",")[2],libr.data("Location2").split(",")[3]};
			String wgtinfo[]={libr.data("weights").split(",")[0],libr.data("weights").split(",")[1]};
			OPR335.deleteShipmentLocation(sccs[0]);
			OPR335.addLooseShipment(pcsinfo[0],wgtinfo[0],locs[0]);
	        OPR335.selectSCC(3,1);
	        OPR335.addLooseShipment();
	        OPR335.addLooseShipment(pcsinfo[1],wgtinfo[1],locs[1]);
	        OPR335.selectSCC(4,2);
	        OPR335.addLooseShipment();
			OPR335.editShipmentLocation(pcsinfo[2],sccs[2],locs[2]);
			OPR335.editShipmentLocation(pcsinfo[3],sccs[3],locs[3]);
			...
			OPR335.verifySUNotNull(cust.data("SCC").split(",")[3]);	
