...
/**Capture Partial Pieces of AWB as Damage Capture for Part Shipment with the Damage Code matched with Auto Block Set up **/


public class IASCB_31368_TC09 extends BaseSetup {
	...
	public void getTestSuite(Map<Object, Object> map) {
		...
		try {
			...
			map.put("pcs1", libr.data("Pieces2").split(",")[0]);
			map.put("wgt1", libr.data("Weight2").split(",")[0]);
			map.put("vol1", libr.data("Volume2").split(",")[0]);
			map.put("ShpmntDesc1", libr.data("ShipmentDesc").split(",")[0]);
			
			map.put("pcs2", libr.data("Pieces2").split(",")[1]);
			map.put("wgt2", libr.data("Weight2").split(",")[1]);
			map.put("vol2", libr.data("Volume2").split(",")[1]);
			map.put("ShpmntDesc2", libr.data("ShipmentDesc").split(",")[1]);
			


			/***** loading XFBL ***/ 

			//Create XFBL message 1
			map.put("FullFlightNumber", cust.data("FullFlightNo"));

			cust.createXMLMessage("MessageExcelAndSheetXFBL", "MessageParamXFBL");
			String shipment[] = { libr.data("FullAWBNo") + ";" + libr.data("pcs1") + ";" + libr.data("wgt1")+ ";"
					+ libr.data("vol1") + ";" + libr.data("ShpmntDesc1")};
			...
			cust.createXMLMessage("MessageExcelAndSheetXFBL", "MessageParamXFBL");
			String shipment1[] = { libr.data("FullAWBNo") + ";" + libr.data("pcs2") + ";" + libr.data("wgt2")+ ";"
					+ libr.data("vol2") + ";" + libr.data("ShpmntDesc2")};
			String scc1[] = { cust.data("SCC").split(",")[1] };

			cust.createXFBLMessage("XFBL_2", shipment1, scc1, routing);
			Cgocxml.sendMessageCgoCXML("ICARGO");
			...
			//part shipment acceptance
			cust.searchScreen("OPR335", "Goods Acceptance");
			cust.listAWB("AWBNo", "prop~CarrierNumericCode", "Goods Acceptance");
			OPR335.enterLooseShipmentDetails("Location", "pcs1", "wgt1");
			OPR335.selectSCC(2,0);
			OPR335.addLooseShipment();
			OPR335.addNewLooseAcceptance("Location", "pcs2", "wgt2");
			OPR335.selectSCC(3,1);
			OPR335.addLooseShipment();
			...
			//verify build up allowed for part2 shipment
			cust.searchScreen("OPR344", "Export manifest");
			OPR344.listFlight("prop~flight_code", "FlightNo2","StartDate");
			String uldNum1=cust.create_uld_number("UldType", "prop~flight_code");
			map.put("uldNum2", uldNum1);
			OPR344.addULDWithoutAWB("uldNum2","0");
			OPR344.clickShipemntFromPlannedSection("AWBNo");
			OPR344.selectULD("uldNum2");
			OPR344.enterPiecesToBeCheckedOut(1, "pcs2");
			OPR344.clickBuildUpComplete();
			OPR344.verifyBuildUpComplete("uldNum2");
			cust.closeTab("OPR344", "Export manifest");

			/**** TRC006 - Tracing Reports ******/
			//Creating block for part1 shipments
			...
			TRC006.addDamageDetails("DmgCode", "pcs1", "DamageDetails");
			TRC006.printDamage();
			TRC006.closeTab("TRC006", "Tracing Reports");


			/*****OPR023 - AWB Clearance *******/            
			//Verify block created for part1 shipments
			...
			/**** OPR344 - Export manifest****/
			//verify build up not allowed for part1 shipment
			cust.searchScreen("OPR344", "Export manifest");
			...
			cust.searchScreen("OPR339", "Security and Screening");
			OPR339.listAWBNo("AWBNo", "CarrierNumericCode", "OPR339 - Security & Sceening");
			OPR339.enterScreeningDetails("ScreeningMethod","pcs1","wgt1","val~Pass"); 
			OPR339.saveSecurityDetails();
			cust.closeTab("OPR339", "Security & Sceening");
			...
			cust.searchScreen("OPR339", "Security and Screening");
			OPR339.listAWBNo("AWBNo", "CarrierNumericCode", "OPR339 - Security & Sceening");
			OPR339.enterScreeningDetails("ScreeningMethod2","pcs1","wgt1","val~Pass"); 
			OPR339.saveSecurityDetails();
			cust.closeTab("OPR339", "Security & Sceening");


			/*****OPR023 - AWB Clearance *******/            
			//Verify that block is released for part1 shipment
			cust.searchScreen("OPR023", "AWB Clearance");
			OPR023.listAWB("prop~CarrierNumericCode","prop~AWBNo");
			...
			/**** OPR344 - Export manifest****/
			//verify build up for part1 shipments
			cust.searchScreen("OPR344", "Export manifest");
			OPR344.listFlight("prop~flight_code", "FlightNo","StartDate");
			OPR344.clickShipemntFromPlannedSection("AWBNo");
			OPR344.selectULD("uldNum1");
			OPR344.clickBuildUpComplete();
			OPR344.verifyBuildUpComplete("uldNum1");
			cust.closeTab("OPR344", "Export manifest");
