...
/**** Disruption management process when ASM Cancel is processed.****/

public class TC027 extends BaseSetup {
	...
	public void getTestSuite(Map<Object, Object> map) throws InterruptedException {
		...
		try {
          ...
          /** SHR003 - Maintain Aircraft Type- Take aircraft capacity **/
			...
			String startDate1 = customfunctions.createDateFormat("dd-MMM-yyyy", 1, "DAY", "");
			...
			customfunctions.searchScreen("FLT005", "FLT005 - Maintain Flight Schedule");
			FLT005.listNewFlight("carrierCode","prop~flightNo2", startDate1, startDate1,"FullFlightNumber");
    		customfunctions.closeTab("FLT005", "Maintain Schedule");
			...
			customfunctions.createTextMessage("MessageExcelAndSheetASM3", "MessageParamASM3");
			customfunctions.searchScreen("MSG005", "MSG005 - List Messages");
			MSG005.loadFromFile("All", "ALL", "JMS", "", "Origin", "", "ASM_NEW");
			...
			map.put("pmkey", "NEW"+" - "+customfunctions.data("carrierCode")+" - "+customfunctions.data("FlightNo2")+" - "+customfunctions.data("ASMStartDate").toUpperCase());
			MSG005.clickCheckBox("pmkey");
			MSG005.clickprocess();
			customfunctions.closeTab("MSG005", "List Message");
			
			/** CAP147 -Monitor Flights f1**/ 
			
            customfunctions.searchScreen("CAP147", "Monitor Flights");
			CAP147.listFlight("carrierCode", "FlightNo", "flightDate");
			CAP147.verifyFlightStatus("val~TO BE CANCELLED");
			CAP147.selectAWBFromConfirmedBkg("prop~AWBNo");
			CAP147.selectOptions("Reassign");
			CAP147.selectFlightfromPopup("Origin", "Destination", "StartDate", "Aircraft", customfunctions.data("FullFlightNo2"),"1", false);
			
			//capture irregularity step to be added
			customfunctions.closeTab("CAP147", "Monitor Flights");

			/** CAP147 -Monitor Flights f1 - RELIST AND CHECK THE CAPACITY DETAILS OF F1**/ 
