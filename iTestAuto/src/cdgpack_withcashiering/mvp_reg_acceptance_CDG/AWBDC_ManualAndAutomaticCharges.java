...
public class AWBDC_ManualAndAutomaticCharges extends BaseSetup{
	...
	public void getTestSuite(Map<Object, Object> map) {
		...
		try {
			...
			OPR026.storeOtherChargesValue("OtherCharges","IATARate");
			OPR026.enterChargeCode(1,"OtherCharges2");
			...
			String expValues[]={cust.data("OtherCharges")+"="+cust.data("IATARate"),cust.data("OtherCharges2")+"="+cust.data("OCValue")};
			cust.compareMaps(hm, expValues,"OPR026","Other Charges");
		    
			
			//Click Notify button and enter notification details		
			OPR026.updateNfyDetails("NotifyCode");
			OPR026.saveAWB(); 
            ...
            cust.searchScreen("OPR026","Capture AWB");
            OPR026.listAWB("prop~AWBNo", "prop~CarrierNumericCode");
			OPR026.captureCheckSheet(false);
			 //As Is Execute AWB
			OPR026.asIsExecute();
			cust.closeTab("OPR026", "Capture AWB");
         ...
         "\n"+"</ApplicableLogisticsAllowanceCharge>");
			...
			msgContents.add("val~<ApplicableLogisticsAllowanceCharge>"+"\n"+
			         "<ID>"+cust.data("OtherCharges")+"</ID>"+
			    	     "\n"+"<PrepaidIndicator>true</PrepaidIndicator>"+
			         "\n"+"<PartyTypeCode>C</PartyTypeCode>" +
			    	     "\n"+"<ActualAmount currencyID="+curr+">"+cust.data("IATARate")+"</ActualAmount>"+
			         "\n"+"</ApplicableLogisticsAllowanceCharge>");
